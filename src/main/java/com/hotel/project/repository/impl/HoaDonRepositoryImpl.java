...
//	------------------------------Xử lý thống kê---------------------------------
	...
	@Override
	public Map<LocalDateTime, Double> findMaxByThoiGianLHD() {
		Map<LocalDateTime, Double> map=new HashMap<LocalDateTime, Double>();
		Aggregation aggregation=
				Aggregation.newAggregation(Aggregation.group("thoiGianLHD").sum("thanhTien").as("sum"));
		mongoTemplate.aggregate(aggregation, HoaDon.class, Map.class).getMappedResults().forEach(l->{
		});;
		return null;
	}
